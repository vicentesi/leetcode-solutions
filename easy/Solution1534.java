        ...
        for (int i = 0; i < arr.length - 2; i++) {
            for (int j = i + 1; j < arr.length - 1; j++) {
                for (int k = j + 1; k < arr.length; k++) {
                    int aAbs = Math.abs(arr[i] - arr[j]);
                    int bAbs = Math.abs(arr[j] - arr[k]);
                    int cAbs = Math.abs(arr[i] - arr[k]);

                    if (aAbs <= a && bAbs <= b && cAbs <= c)
                        count++;
                }
            }
        }
...
public class Solution1534 {
    public static void main(String[] args) {
        ...
        int b = 2;
        int c = 3;
        System.out.println(solution.countGoodTriplets(arr, a, b, c)); // Output: 4
    }
}
