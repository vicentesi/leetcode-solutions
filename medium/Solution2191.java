...
class Solution2191 {
  public int[] sortJumbled(int[] mapping, int[] nums) {
    ...
    for (int i = 0; i < nums.length; i++) {
      String mappedNum = "";
      String numChars = String.valueOf(nums[i]);
      int nLength = numChars.length();

      for (int j = 0; j < nLength; j++) {
        mappedNum += mapping[Character.getNumericValue(numChars.charAt(j))];
      }

      map.put(nums[i], Integer.parseInt(mappedNum));
    }

    Integer[] numsNp = Arrays.stream(nums).boxed().toArray(Integer[]::new);

    Arrays.sort(numsNp, (a, b) -> Integer.compare(map.get(a), map.get(b)));

    return Arrays.stream(numsNp).mapToInt(a -> a).toArray();
  }

  public static void main(String[] args) {
    Solution2191 mySolution = new Solution2191();
    ...
    String output = Arrays.toString(mySolution.sortJumbled(mapping, nums));
